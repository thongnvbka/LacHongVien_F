  ...
  public  class NewsDAL : BaseDALClass<News>
    {
        ...
        public async Task<PagedResult<News>> GetListNewsbyCates(int id, int page, int pageSize)
        {
            using (var connection = CreateConnection())
            {
                if (connection.State == System.Data.ConnectionState.Closed)
        ...
        public async Task<IEnumerable<News>> GetListTopMaumo(int top)
        {
            using (var connection = CreateConnection())
            {
                if (connection.State == System.Data.ConnectionState.Closed)
                ...
                var paramaters = new DynamicParameters();
                paramaters.Add("@top", top);
                var result = await connection.QueryAsync<News>("news_list_top_maumo", paramaters, null, null, System.Data.CommandType.StoredProcedure);
                return result;
            }
        }

        public async Task<IEnumerable<News>> GetListTopDichVu(int top)
        {
            using (var connection = CreateConnection())
            {
                if (connection.State == System.Data.ConnectionState.Closed)
                ...
                var paramaters = new DynamicParameters();
                paramaters.Add("@top", top);
                var result = await connection.QueryAsync<News>("news_list_top_dichvu", paramaters, null, null, System.Data.CommandType.StoredProcedure);
                return result;
            }
        }
